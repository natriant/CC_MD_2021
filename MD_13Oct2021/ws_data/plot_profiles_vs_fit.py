...
for i in range(len(y_dict[f'positions_Set1'])): # iterate over the acquisitions for each setting
    fig, ax = plt.subplots()
    ...
    for my_set in range(1,3):
        ...
        ax.plot(xRef, yRef, 'o', ms=8, c=f'C{my_set-1}', ls='--', label=r'$\mathrm{\epsilon_y}$'+f',Set {my_set}={ey:.3f}' +r'$\pm$'+ f' {err_y:.3f}'+ r'$\mathrm{[\mu m}]$')
...
for i in range(len(x_dict[f'positions_Set1'])): # iterate over the acquisitions for each setting
    ...
    for my_set in range(1,3):
        ...
        results = x_dict[f'Set{my_set}_myFitresults'][i]
        A, mu, sigma, offset = results[0], results[1], results[2], results[3]

        xRef = x_dict[f'positions_Set{my_set}'][i]-mu
        yRef = x_dict[f'profiles_Set{my_set}'][i]-offset

        k = yRefFun(xRef, A, 0, sigma, 0)
        max_to_normalize = max(k)

        ex, err_x = x_dict[f'emittance_Set{my_set}_myFit'][i][0], x_dict[f'emittance_Set{my_set}_myFit'][i][1]
        ax.plot(xRef, yRef, 'o', ms=8, c=f'C{my_set-1}', ls ='--', label=r'$\mathrm{\epsilon_x}$'+f',Set {my_set}={ex:.3f}' +r'$\pm$'+ f' {err_x:.3f}'+ r'$\mathrm{[\mu m}]$')
        ax.plot(xRef, k, c=f'C{my_set-1}')
        ax.set_xlim(-5, 5)
        ax.set_ylim(-10, 1800)
    ...
    plt.grid(ls='--')
    plt.tight_layout()
    plt.savefig(f'SPS.BWS.51637.H_{title[10:19]}.png', bbox_inches='tight')
